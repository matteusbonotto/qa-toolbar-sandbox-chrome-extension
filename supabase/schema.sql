...
create table if not exists public.engagement_campaign_submissions (
  ...
  social_post_url text not null check (social_post_url ~* '^https?://'),
  linkedin_post_url text not null check (linkedin_post_url ~* '^https?://'),
...
-- "Limpar cache e cookies do site": basic QA housekeeping, same tier as Gravador de Passos
-- (available from the entry Smoke Test plan upward).
insert into public.features (key, value_type, description) values
