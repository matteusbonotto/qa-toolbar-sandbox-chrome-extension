...
--   npm run backend:bootstrap -- -ProjectRef <20-char-ref> -DatabaseUrl <postgres-url>
-- If this file was applied manually in the SQL Editor, rerun the same command
-- with -SkipSchema and omit -DatabaseUrl; price registration and function deploy
-- still happen through authenticated Supabase APIs.
...
begin
  ...
  values (new.id, now(), now() + interval '30 days', 'QTS-' || upper(encode(gen_random_bytes(4), 'hex')))
...
-- ============================================================================
-- Helper: is the current caller a founder? (used throughout the RLS below)
-- ============================================================================
...
as $$
  select exists (
    ...
    where ur.user_id = auth.uid() and r.key = 'founder'
  );
$$;
...
begin
  ...
  return query
    select u.id, u.email, p.display_name, p.trial_ends_at, p.created_at
    from auth.users u
...
-- api_rate_limits: no client access at all (service-role only, bypasses RLS)
...
-- 2. Call `select public.bootstrap_founder();` as that user (e.g. from the
--    admin app right after first login, or via the SQL editor while
--    impersonating that session) — this is the ONLY way the founder role
--    is ever granted, and it only works for that exact email.
