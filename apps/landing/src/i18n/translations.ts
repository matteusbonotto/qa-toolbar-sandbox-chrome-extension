...
export interface Dictionary {
  ...
  privacy: {
    ...
    dataCategoriesTitle: string;
    dataCategoriesBody: string;
...
const pt: Dictionary = {
  ...
  features: {
    ...
    items: {
      ...
      breakpointViewer: {
        title: "Simulador de dispositivos",
      ...
      languageValidator: {
        title: "Validador de textos/i18n",
    ...
    plans: {
      ...
      "regression-runner": {
        ...
        features: [
          ...
          "Validador de campos e Auto preenchimento (dados de teste sintéticos)",
        ],
      },
      "root-cause-analyst": {
  ...
  privacy: {
    ...
    effectiveDate: "Versão vigente desde 26 de julho de 2026.",
    ...
    sharingBody: "Nunca vendemos, alugamos ou compartilhamos seus dados com anunciantes, corretores de dados ou para publicidade ou rastreamento. Supabase recebe e-mail, credenciais de autenticação, sessão, assinatura, vouchers e dados de campanhas para autenticar a conta e operar o backend. Stripe recebe diretamente os dados reais de pagamento, e-mail e informações necessárias ao checkout, à assinatura, a reembolsos e à prevenção de fraude. Resend recebe o endereço de e-mail e o conteúdo operacional necessário para entregar confirmações, recuperação de senha, códigos de segurança e avisos de pagamento. GitHub Pages hospeda a página pública e pode processar dados técnicos de conexão, como endereço IP e registros HTTP, conforme a política do GitHub. Esses fornecedores atuam somente para prestar o serviço e segundo seus próprios termos e políticas. Também podemos compartilhar o mínimo necessário por obrigação legal, proteção contra fraude ou segurança. Conteúdo de páginas testadas, screenshots, credenciais sandbox e meios de pagamento sandbox não são compartilhados com esses fornecedores.",
  ...
  trust: {
    ...
    badgesTitle: "Por que você pode confiar",
    badges: [
      { title: "RLS por padrão no banco", body: "Toda tabela do Postgres nega acesso por padrão; políticas explícitas liberam só o que cada papel (você, o founder, o backend) precisa enxergar." },
      { title: "Pagamento 100% no Stripe", body: "Número de cartão nunca passa pelos nossos servidores nem pela extensão - o checkout é hospedado pelo Stripe, que cuida da conformidade PCI." },
      { title: "Extensão local-first", body: "Seu workspace de testes (clientes, projetos, credenciais sandbox) fica no seu navegador. Só falamos com o backend para autenticar você e checar seu plano." },
      { title: "Exclusão sob seu controle", body: "Excluir a conta cancela a assinatura, apaga seus dados pessoais na hora e anonimiza (nunca apaga) apenas os registros financeiros exigidos por lei." },
    ],
...
const es: Dictionary = {
    ...
    plans: {
      ...
      "regression-runner": {
        ...
        features: [
          ...
          "Validador de campos y Auto preenchimento (datos de prueba sintéticos)",
        ],
      },
      "root-cause-analyst": {
        name: "Root Cause Analyst",
  ...
  privacy: {
    ...
    effectiveDate: "Versión vigente desde el 26 de julio de 2026.",
    ...
    sharingBody: "Nunca vendemos, alquilamos ni compartimos tus datos con anunciantes, corredores de datos o para publicidad o rastreo. Supabase recibe correo, credenciales de autenticación, sesión, suscripción, vouchers y datos de campañas para autenticar la cuenta y operar el backend. Stripe recibe directamente datos reales de pago, correo e información necesaria para checkout, suscripciones, reembolsos y prevención de fraude. Resend recibe el correo y el contenido operativo necesario para enviar confirmaciones, recuperación de contraseña, códigos de seguridad y avisos de pago. GitHub Pages aloja el sitio público y puede procesar datos técnicos de conexión, como dirección IP y registros HTTP, conforme a la política de GitHub. También podemos compartir el mínimo necesario por obligación legal, fraude o seguridad. Contenido de páginas probadas, capturas, credenciales sandbox y medios de pago sandbox no se comparten con estos proveedores.",
  ...
  trust: {
    ...
    badgesTitle: "Por qué puedes confiar",
    badges: [
      { title: "RLS por defecto en la base de datos", body: "Toda tabla de Postgres deniega el acceso por defecto; políticas explícitas habilitan solo lo que cada rol (tú, el founder, el backend) necesita ver." },
      { title: "Pago 100% en Stripe", body: "El número de tarjeta nunca pasa por nuestros servidores ni por la extensión: el checkout está alojado por Stripe, que se encarga del cumplimiento PCI." },
      { title: "Extensión local-first", body: "Tu workspace de pruebas (clientes, proyectos, credenciales sandbox) queda en tu navegador. Solo hablamos con el backend para autenticarte y comprobar tu plan." },
      { title: "Eliminación bajo tu control", body: "Eliminar la cuenta cancela la suscripción, borra tus datos personales al instante y anonimiza (nunca borra) solo los registros financieros exigidos por ley." },
    ],
...
const en: Dictionary = {
    ...
    plans: {
      ...
      "regression-runner": {
        ...
        features: [
          ...
          "Validador de campos and Auto preenchimento (synthetic test data)",
        ],
      },
      "root-cause-analyst": {
        name: "Root Cause Analyst",
  ...
  privacy: {
    ...
    effectiveDate: "Effective version dated July 26, 2026.",
    ...
    sharingBody: "We never sell, rent, or share your data with advertisers or data brokers, or for advertising or tracking. Supabase receives email, authentication credentials, session, subscription, voucher, and campaign data to authenticate accounts and operate the backend. Stripe directly receives real payment data, email, and information required for checkout, subscriptions, refunds, and fraud prevention. Resend receives an email address and the operational message content required to deliver confirmations, password recovery, security codes, and payment notices. GitHub Pages hosts the public website and may process technical connection data such as IP addresses and HTTP logs under GitHub's policy. We may also share the minimum required by law or for fraud and security protection. Tested-page content, screenshots, sandbox credentials, and sandbox payment methods are not shared with these providers.",
  ...
  trust: {
    ...
    badgesTitle: "Why you can trust it",
    badges: [
      { title: "Deny-by-default RLS", body: "Every Postgres table denies access by default; explicit policies only unlock what each role (you, the founder, the backend) actually needs to see." },
      { title: "Payment 100% on Stripe", body: "Your card number never touches our servers or the extension - checkout is hosted by Stripe, which handles PCI compliance." },
      { title: "Local-first extension", body: "Your testing workspace (clients, projects, sandbox credentials) stays in your browser. We only talk to the backend to authenticate you and check your plan." },
      { title: "Deletion, on your terms", body: "Deleting your account cancels the subscription, erases your personal data immediately, and only anonymizes (never erases) the financial records the law requires us to keep." },
    ],
