...
function releaseNotesCopy() {
  ...
  if (language.startsWith("es")) return { title: `Actualizado a la versión ${state.pendingReleaseNote?.version || ""}`, intro: "Tus datos y configuraciones anteriores se conservaron.", items: ["Inspectors: las tarjetas ahora muestran el endpoint completo junto al método y al status.", "Los campos de búsqueda (Inspectors, Error Monitor, Cuentas de prueba, Métodos de pago, Recursos) ya no pierden el foco a cada tecla escrita.", "La grabación en GIF ahora captura en mayor resolución (1280x720), dejando el texto legible.", "Las herramientas bloqueadas por tu plan ahora aparecen con un candado en el menú Tools, en vez de simplemente desaparecer."], action: "Entendido" };
  if (language.startsWith("en")) return { title: `Updated to version ${state.pendingReleaseNote?.version || ""}`, intro: "Your existing data and settings were preserved.", items: ["Inspectors: cards now show the full endpoint alongside method and status.", "Search fields (Inspectors, Error Monitor, Test Accounts, Payment Methods, Resources) no longer lose focus on every keystroke.", "GIF recording now captures at a higher resolution (1280x720), keeping text legible.", "Tools locked by your plan now show a lock badge in the Tools menu instead of simply disappearing."], action: "Got it" };
  return { title: `Atualizado para a versão ${state.pendingReleaseNote?.version || ""}`, intro: "Seus dados e configurações anteriores foram preservados.", items: ["Inspectors: os cards agora mostram o endpoint completo junto com método e status.", "Os campos de busca (Inspectors, Error Monitor, Contas de teste, Meios de pagamento, Recursos) não perdem mais o foco a cada tecla digitada.", "A gravação em GIF agora captura em resolução mais alta (1280x720), deixando o texto legível.", "Ferramentas bloqueadas pelo seu plano agora aparecem com um cadeado no menu Tools, em vez de simplesmente sumir."], action: "Entendi" };
}
