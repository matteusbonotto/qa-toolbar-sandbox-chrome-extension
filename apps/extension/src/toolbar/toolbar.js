...
function releaseNotesCopy() {
  ...
  if (language.startsWith("es")) return { title: `Actualizado a la versión ${state.pendingReleaseNote?.version || ""}`, intro: "Tus datos y configuraciones anteriores se conservaron.", items: ["Nuevo Pixel Perfect: líneas guía, regla inteligente e inspector de elementos (pasa el mouse, usa el scroll para subir/bajar en el DOM, clic para fijar).", "Grabador de pasos reescrito: frases explícitas y Gherkin real (Dado/Y/Cuando/Entonces).", "Tour y tutorial ahora usan un sitio de demostración propio, sin depender de terceros.", "Vídeos MP4 reproducibles y GIF real en partes de 15 segundos.", "Entornos de prueba y producción separados."], action: "Entendido" };
  if (language.startsWith("en")) return { title: `Updated to version ${state.pendingReleaseNote?.version || ""}`, intro: "Your existing data and settings were preserved.", items: ["New Pixel Perfect: guide lines, a smart ruler, and an element inspector (hover, scroll to walk the DOM up/down, click to pin).", "Rewritten Step Recorder: explicit sentences and real Gherkin (Given/And/When/Then).", "The tour and tutorial now use our own demo site, no longer depending on a third party.", "Playable MP4 and real GIF split into 15-second parts.", "Separated test and production environments."], action: "Got it" };
  return { title: `Atualizado para a versão ${state.pendingReleaseNote?.version || ""}`, intro: "Seus dados e configurações anteriores foram preservados.", items: ["Novo Pixel Perfect: linhas guia, régua inteligente e inspetor de elementos (passe o mouse, role o scroll pra subir/descer no DOM, clique pra fixar).", "Gravador de Passos reescrito: frases explícitas e Gherkin de verdade (Dado/E/Quando/Então).", "O tour e o tutorial agora usam um site de demonstração próprio, sem depender de terceiros.", "Vídeos MP4 reproduzíveis e GIF real dividido em partes de 15 segundos.", "Ambientes de teste e produção separados."], action: "Entendi" };
}
