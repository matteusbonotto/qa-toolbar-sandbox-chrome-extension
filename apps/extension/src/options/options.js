...
async function showPendingReleaseNotes() {
  ...
  const texts = currentLocale.startsWith("en")
    ? { title: `Updated to version ${note.version}`, intro: "Your previous data and settings were preserved.", done: "Got it", items: ["New color identity across the extension, website and Admin", "Workspace relationships now show complete names, logos and initials without clipped accordions", "Mobile sidebar and toolbar position previews now match the selected edge", "Workspace JSON export and import preserve uploaded and URL-based images", "Search fields keep focus and preserve the open sidebar while you type"] }
    : currentLocale.startsWith("es")
      ? { title: `Actualizado a la versión ${note.version}`, intro: "Tus datos y configuraciones se conservaron.", done: "Entendido", items: ["Nueva identidad a color en la extensión, el sitio y el Admin", "Las relaciones del Workspace ahora muestran nombres completos, logos e iniciales sin accordions recortados", "Las vistas previas mobile del sidebar y la toolbar ahora corresponden a la posición seleccionada", "La exportación e importación JSON del Workspace conserva imágenes subidas o registradas por URL", "Las búsquedas conservan el foco y el sidebar abierto mientras escribes"] }
      : { title: `Atualizado para a versão ${note.version}`, intro: "Seus dados e configurações anteriores foram preservados.", done: "Entendi", items: ["Nova identidade colorida na extensão, no site e no Admin", "Relações do Workspace agora exibem nomes completos, logos e iniciais sem accordions cortados", "Prévias mobile de sidebar e toolbar agora correspondem à posição selecionada", "Exportação e importação JSON do Workspace preservam imagens enviadas ou cadastradas por URL", "Buscas mantêm o foco e preservam o sidebar aberto enquanto você digita"] };
