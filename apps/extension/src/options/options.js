...
async function showPendingReleaseNotes() {
  ...
  const texts = currentLocale.startsWith("en")
    ? { title: `Updated to version ${note.version}`, intro: "Your previous data and settings were preserved.", done: "Got it", items: ["Menu names now clearly describe each module", "Saved macros appear in their submenu", "Endpoint Observer keeps history, exports CSV, and supports clearing it", "Reports generates an A4 view ready to save as PDF", "On mobile, Pass and Fail are grouped with Warning and Question"] }
    : currentLocale.startsWith("es")
      ? { title: `Actualizado a la versión ${note.version}`, intro: "Tus datos y configuraciones se conservaron.", done: "Entendido", items: ["Los nombres del menú ahora describen claramente cada módulo", "Macros guardadas aparecen en su submenú", "El Observador de endpoints conserva historial, exporta CSV y permite limpiarlo", "Informes genera una vista A4 lista para guardar como PDF", "En móvil, Pass y Fail están junto a Warning y Question"] }
      : { title: `Atualizado para a versão ${note.version}`, intro: "Seus dados e configurações anteriores foram preservados.", done: "Entendi", items: ["Os nomes do menu agora descrevem claramente cada módulo", "Macros salvas aparecem em seu submenu", "O Observador de endpoints mantém histórico, exporta CSV e permite limpar os registros", "Relatórios gera uma visualização A4 pronta para salvar como PDF", "No mobile, Pass e Fail ficam junto de Warning e Question"] };
