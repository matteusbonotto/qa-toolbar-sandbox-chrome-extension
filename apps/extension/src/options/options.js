...
async function showPendingReleaseNotes() {
  ...
  const texts = currentLocale.startsWith("en")
    ? { title: `Updated to version ${note.version}`, intro: "Your previous data and settings were preserved.", done: "Got it", items: ["Inspectors cards now show the full endpoint alongside method and status", "Search fields no longer lose focus on every keystroke", "GIF recording now captures at a higher, legible resolution (1280x720)", "Plan-locked tools now show a lock badge instead of disappearing"] }
    : currentLocale.startsWith("es")
      ? { title: `Actualizado a la versión ${note.version}`, intro: "Tus datos y configuraciones se conservaron.", done: "Entendido", items: ["Las tarjetas de Inspectors ahora muestran el endpoint completo junto al método y al status", "Los campos de búsqueda ya no pierden el foco a cada tecla escrita", "La grabación en GIF ahora captura en mayor resolución, legible (1280x720)", "Las herramientas bloqueadas por el plan ahora muestran un candado en vez de desaparecer"] }
      : { title: `Atualizado para a versão ${note.version}`, intro: "Seus dados e configurações anteriores foram preservados.", done: "Entendi", items: ["Cards do Inspectors agora mostram o endpoint completo junto com método e status", "Campos de busca não perdem mais o foco a cada tecla digitada", "Gravação em GIF agora captura em resolução mais alta e legível (1280x720)", "Ferramentas bloqueadas pelo plano agora mostram um cadeado em vez de sumir"] };
