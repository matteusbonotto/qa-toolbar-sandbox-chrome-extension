...
export function SetupWizard({ maximumUrls, onMessage }: { maximumUrls: number; onMessage: (message: string) => void }) {
  ...
  const canLeaveStep = () => {
    if (step === 0 && data.projectName.trim().length < 2) return "Informe o nome do projeto, por exemplo Loja Demo.";
  ...
  return <div className="qtsWizard">
    ...
    <section className="qtsWizardCard">
      {step === 0 && <><WizardHead title="Qual projeto você vai testar?" text="Use um nome reconhecível por todo o time, como Checkout, Portal do Cliente ou Loja Demo." /><label className="qtsWizardField">Nome do projeto <Info text="Este nome aparece na toolbar e organiza ambientes, contas e evidências." /><input autoFocus value={data.projectName} onChange={(event) => setData({ ...data, projectName: event.target.value })} placeholder="Ex.: Loja Demo" /></label><div className="qtsImageGrid"><ImageField label="Imagem do projeto" value={data.projectImage} onChange={(value) => setData({ ...data, projectImage: value })} /><ImageField label="Imagem do cliente" value={data.clientImage} onChange={(value) => setData({ ...data, clientImage: value })} /><ImageField label="Imagem da assinatura" value={data.subscriptionImage} onChange={(value) => setData({ ...data, subscriptionImage: value })} /></div></>}
